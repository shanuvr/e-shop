...
export default function SuperAdminSidebar() {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="lg:hidden bg-slate-950 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-40">
        <Link to="/" className="flex items-center gap-2">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold">
            <ShieldAlert className="w-4 h-4" />
          </div>
          <span className="font-black text-lg tracking-tight">E-SHOP <span className="text-blue-400 text-xs">SUPER ADMIN</span></span>
        </Link>
        <button
        ...
        <div>
          ...
          <div className="p-5 border-b border-slate-800/80">
            <Link to="/" className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="font-black text-white text-base tracking-tight leading-none">E-SHOP HQ</h1>
                <span className="text-[10px] font-semibold text-blue-400 tracking-wide">Super Admin Portal</span>
              </div>
            </Link>
          </div>
          ...
          <nav className="p-3 space-y-1.5 mt-2">
            <div className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">HQ Operations</div>
            {menuItems.map((item) => {
